...
@Component({
  ...
  template: `
    <div
      class="bg-surface text-on-background antialiased flex items-center justify-center min-h-screen p-6"
    >
      <!-- Background decoration -->
      <div
        class="fixed top-0 right-0 -z-10 w-[40vw] h-[40vw] bg-primary-fixed/30 rounded-full blur-[120px] translate-x-1/2 -translate-y-1/2"
      ></div>
      <div
        class="fixed bottom-0 left-0 -z-10 w-[30vw] h-[30vw] bg-surface-container-high/50 rounded-full blur-[100px] -translate-x-1/2 translate-y-1/2"
      ></div>
      ...
      <main class="w-full max-w-[400px] flex flex-col gap-8">
        ...
        <header class="flex flex-col items-center text-center space-y-2">
          <div class="flex items-center gap-2 mb-2">
            <div
              class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary-container flex items-center justify-center shadow-lg shadow-primary/20"
            >
              <span
        ...
        <!-- Card -->
        <div
          class="bg-surface-container-lowest rounded-xl p-8 shadow-[0_24px_48px_-12px_rgba(19,27,46,0.04)] border border-outline-variant/15"
        >
          <!-- Session expiry notice -->
          <div
            *ngIf="sessionExpired"
            class="flex items-center gap-2 bg-[#fffbeb] p-3 rounded-lg border border-[#fde68a] mb-6"
          >
          ...
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-6">
            <!-- Email -->
            <div class="space-y-2">
              <label class="block text-sm font-semibold text-on-surface-variant px-1" for="email"
                >Email</label
              >
              <input
                id="email"
              ...
              <span
                *ngIf="form.get('email')?.touched && form.get('email')?.hasError('required')"
                class="block text-xs text-error px-1"
              >
                Email is required.
              </span>
            </div>
          ...
          <div class="mt-8 pt-6 border-t border-outline-variant/10 text-center">
            <p class="text-on-surface-variant text-sm">
              ...
              <a
                routerLink="/register"
                class="font-bold text-primary hover:text-primary-container transition-colors ml-1"
                >Register</a
              >
            </p>
          </div>
